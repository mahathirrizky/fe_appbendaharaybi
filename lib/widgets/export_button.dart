   ...
   CashflowBloc alurdanaA = context.watch<CashflowBloc>();
    ...
    return StreamBuilder<List<CashflowModel>>(
      ...
      builder: (context, snapshot) {
        ...
        return BlocConsumer<ExporttoexcelBloc, ExporttoexcelState>(
          listener: (context, state) {
            if (state is SuccessState) {
              // Show SnackBar when export is successful
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('Export to Excel Berhasil!'),
                ),
              );
            }
          },
          builder: (context, state) {
            return GestureDetector(
              onTap: () {
                final exporttoexcelBloc = context.read<ExporttoexcelBloc>();
                exporttoexcelBloc.add(
                  ExporttoexcelEventCreate(
                    // widget.namasheet,
